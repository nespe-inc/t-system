<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('調査情報編集') }} - {{ $company->company_name }}
            </h2>
            <div class="flex items-center space-x-3">
                <a href="{{ route('companies.investigations', $company) }}">
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('companies.investigations.update', [$company, $investigation]) }}" class="space-y-8">
                @csrf
                @method('PUT')
                    ...
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <label class="flex items-center cursor-pointer">
                                <input type="radio" name="investigation_type" id="check-teikoku" class="investigation-radio mr-2" data-target="investigation-teikoku" value="teikoku" {{ $selectedType == 'teikoku' ? 'checked' : '' }}>
                                <span class="text-gray-700 dark:text-gray-300">帝国調査履歴</span>
                            </label>
                            <label class="flex items-center cursor-pointer">
                                <input type="radio" name="investigation_type" id="check-tosho" class="investigation-radio mr-2" data-target="investigation-tosho" value="tosho" {{ $selectedType == 'tosho' ? 'checked' : '' }}>
                                <span class="text-gray-700 dark:text-gray-300">東商調査履歴</span>
                            </label>
                            <label class="flex items-center cursor-pointer">
                                <input type="radio" name="investigation_type" id="check-seni" class="investigation-radio mr-2" data-target="investigation-seni" value="seni" {{ $selectedType == 'seni' ? 'checked' : '' }}>
                                <span class="text-gray-700 dark:text-gray-300">繊維調査履歴</span>
                            </label>
                            <label class="flex items-center cursor-pointer">
                                <input type="radio" name="investigation_type" id="check-kensetsu" class="investigation-radio mr-2" data-target="investigation-kensetsu" value="kensetsu" {{ $selectedType == 'kensetsu' ? 'checked' : '' }}>
                ...
                <!-- 調査履歴 -->
                <div id="investigation-teikoku" class="investigation-form" style="display: {{ $selectedType == 'teikoku' ? 'block' : 'none' }};">
                    @include('companies.partials.investigations-edit', ['type' => 'teikoku', 'label' => '帝国', 'investigation' => $investigation])
                </div>
                <div id="investigation-tosho" class="investigation-form" style="display: {{ $selectedType == 'tosho' ? 'block' : 'none' }};">
                    @include('companies.partials.investigations-edit', ['type' => 'tosho', 'label' => '東商', 'investigation' => $investigation])
                </div>
                <div id="investigation-seni" class="investigation-form" style="display: {{ $selectedType == 'seni' ? 'block' : 'none' }};">
                    @include('companies.partials.investigations-simple-edit', ['type' => 'seni', 'label' => '繊維', 'investigation' => $investigation])
                </div>
                <div id="investigation-kensetsu" class="investigation-form" style="display: {{ $selectedType == 'kensetsu' ? 'block' : 'none' }};">
                    @include('companies.partials.investigations-simple-edit', ['type' => 'kensetsu', 'label' => '建設', 'investigation' => $investigation])
                </div>
                    ...
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            @php
                                ...
                                $oldRejectionReasons = old('rejection_reasons', $investigation->rejection_reasons ?? []);
                                // 配列であることを確認
                    ...
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-text-input id="amount" class="block mt-1 w-full" type="number" step="0.01" name="amount" :value="old('amount', $investigation->amount ?? '')" />
                                <x-input-error :messages="$errors->get('amount')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="due_date" :value="__('期日')" />
                                <x-text-input id="due_date" class="block mt-1 w-full" type="date" name="due_date" :value="old('due_date', $investigation->due_date ? $investigation->due_date->format('Y-m-d') : '')" />
                                <x-input-error :messages="$errors->get('due_date')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="bill_no" :value="__('手形 No.')" />
                                <x-text-input id="bill_no" class="block mt-1 w-full" type="text" name="bill_no" :value="old('bill_no', $investigation->bill_no ?? '')" />
                                <x-input-error :messages="$errors->get('bill_no')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="bank_branch" :value="__('銀行 支店')" />
                                <x-text-input id="bank_branch" class="block mt-1 w-full" type="text" name="bank_branch" :value="old('bank_branch', $investigation->bank_branch ?? '')" />
                                <x-input-error :messages="$errors->get('bank_branch')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="first_endorsement" :value="__('1裏')" />
                                <x-text-input id="first_endorsement" class="block mt-1 w-full" type="text" name="first_endorsement" :value="old('first_endorsement', $investigation->first_endorsement ?? '')" />
                                <x-input-error :messages="$errors->get('first_endorsement')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="second_endorsement" :value="__('2裏')" />
                                <x-text-input id="second_endorsement" class="block mt-1 w-full" type="text" name="second_endorsement" :value="old('second_endorsement', $investigation->second_endorsement ?? '')" />
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <div class="flex items-center space-x-4 mt-2">
                                    <label class="flex items-center">
                                        <input type="radio" name="client_type" value="A" {{ old('client_type', $investigation->client_type ?? '') == 'A' ? 'checked' : '' }} class="mr-2">
                                        <span class="text-gray-700 dark:text-gray-300">A</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input type="radio" name="client_type" value="旧A" {{ old('client_type', $investigation->client_type ?? '') == '旧A' ? 'checked' : '' }} class="mr-2">
                                        <span class="text-gray-700 dark:text-gray-300">旧A</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input type="radio" name="client_type" value="B" {{ old('client_type', $investigation->client_type ?? '') == 'B' ? 'checked' : '' }} class="mr-2">
                                        <span class="text-gray-700 dark:text-gray-300">B</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input type="radio" name="client_type" value="その他" {{ old('client_type', $investigation->client_type ?? '') == 'その他' ? 'checked' : '' }} class="mr-2">
                                        <span class="text-gray-700 dark:text-gray-300">その他</span>
                                    </label>
                                    <x-text-input class="block w-32" type="text" name="client_other" :value="old('client_other', $investigation->client_other ?? '')" placeholder="その他の場合" />
                                </div>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <x-input-label for="client_company_name" :value="__('依頼人会社名')" />
                                    <x-text-input id="client_company_name" class="block mt-1 w-full" type="text" name="client_company_name" :value="old('client_company_name', $investigation->client_company_name ?? '')" />
                                    <x-input-error :messages="$errors->get('client_company_name')" class="mt-2" />
                                </div>
                                <div>
                                    <x-input-label for="client_address" :value="__('依頼人住所')" />
                                    <textarea id="client_address" name="client_address" rows="2" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">{{ old('client_address', $investigation->client_address ?? '') }}</textarea>
                                    <x-input-error :messages="$errors->get('client_address')" class="mt-2" />
                                </div>
                                <div>
                                    <x-input-label for="client_representative" :value="__('依頼人代表者')" />
                                    <x-text-input id="client_representative" class="block mt-1 w-full" type="text" name="client_representative" :value="old('client_representative', $investigation->client_representative ?? '')" />
                                    <x-input-error :messages="$errors->get('client_representative')" class="mt-2" />
                                </div>
                                <div>
                                    <x-input-label for="client_tel" :value="__('依頼人TEL')" />
                                    <x-text-input id="client_tel" class="block mt-1 w-full" type="text" name="client_tel" :value="old('client_tel', $investigation->client_tel ?? '')" />
